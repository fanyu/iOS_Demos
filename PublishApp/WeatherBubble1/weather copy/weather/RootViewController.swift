...
class RootViewController: UIViewController,CLLocationManagerDelegate {
        ...
        Alamofire.request(.GET, url!).responseJSON() {
            ...
            if error != nil { // cannot get data
                ...
                if let tempResult = json["main"]["temp"].double {
                    ...
                    // get pm2.5
                    var pollution = self.getPollutionInfo(city, token: self.token, stations: self.stations)
                    u?.uCurrentPollution = pollution
                    ...
                    var minUp: Int = NSCalendar.currentCalendar().components(.CalendarUnitMinute, fromDate: NSDate(timeIntervalSince1970: sunrise)).minute
    ...
    func getPollutionInfo(city: String, token: String, stations: String) ->String {
        ...
        Alamofire.request(.GET, url, parameters: params)
            .responseJSON { (request, response, json, error) in
                if let errStr = error {
                    ...
                    println("污染错误信息: \(errStr)")
                    pmStr = "污染错误信息: \(errStr)"
                    //return pmStr
                } else {
                    let json = JSON(json!)
                    self.pm2_5 = json["pm2_5"].intValue
                    pmStr = self.service.pollutionJudge(self.pm2_5)
                    self.pollutionTitle.setTitle("\(pmStr)", forState:.Normal )
                    //println("PM2.5: \(self.pm2_5)")
                }
            }
        return pmStr
    }
