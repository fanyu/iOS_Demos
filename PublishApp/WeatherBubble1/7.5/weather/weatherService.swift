...
public class weatherService {
    ...
    static func hoursTimeFormate(timeInterval: Double) ->String {
        ...
        var language: String = NSLocale.preferredLanguages()[0] as! String
        var time: String?
    ...
    static func shakeToShow(sender: AnyObject, mailFeedBackViewController: MFMailComposeViewController, weiboFeedBackViewController: SLComposeViewController) {
        var alertController = UIAlertController(title: "反馈与点评", message: nil, preferredStyle: UIAlertControllerStyle.ActionSheet)
        var sysInfo = UIDevice.currentDevice().systemVersion
        // email
        var emailFeedBackAction = UIAlertAction(title: "邮件反馈", style: UIAlertActionStyle.Default, handler: { (ok) -> Void in
        ...
        var appleStoreAction = UIAlertAction(title: "我要点评", style: UIAlertActionStyle.Default, handler: { (ok) -> Void in
            var iTunesLink: String = "itms-apps://geo.itunes.apple.com/cn/app/qi-dian-for-wei-bo/id947792507?mt=8"
