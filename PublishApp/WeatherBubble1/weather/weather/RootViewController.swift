...
class RootViewController: UIViewController,CLLocationManagerDelegate, MFMailComposeViewControllerDelegate {
    ...
    @IBAction func longPressButton(sender: AnyObject) {
        let shareControler = weatherService.shareToWeibo(self.view)
    ...
    func feedBackViewControllers() ->(mail: MFMailComposeViewController, weibo: SLComposeViewController) {
        ...
        // weibo
        let weiboViewController: SLComposeViewController = SLComposeViewController(forServiceType: SLServiceTypeSinaWeibo)
