...
class WeChatViewController: UIViewController {
    ...
    private func shareInfo(info: MonkeyKing.Info) {
        ...
        case 0: message = MonkeyKing.Message.WeChat(.Session(info: info))
        case 1: message = MonkeyKing.Message.WeChat(.Timeline(info: info))
        case 2: message = MonkeyKing.Message.WeChat(.Favorite(info: info))
        default:()
        }
        if let message = message {
            MonkeyKing.shareMessage(message, completionHandler: { (result) -> Void in
                print("result: \(result)")
            })
        }
    }
