...
if($_GET["payType"]=="alipay"){
    //支付宝支付的服务器端, 需要填写商户app申请的
    $partner = ""; // 输入自己的 partner
    $seller = ""; // 自己的 seller
...
EOF;
    ...
    $dataString.='&sign_type="RSA"&bizcontext="{"appkey":"2016012101112529"}"&sign="'.$sign.'"'; // 自己的 Appkey
    //生成可以直接打开的链接，让iOS客户端打开：[[UIApplication sharedApplication] openURL:[NSURL URLWithString:$iOSLink]];
    $iOSLink= "alipay://alipayclient/?".urlencode(json_encode(array('requestType' => 'SafePay', "fromAppUrlScheme" => /*iOS App的url schema，支付宝回调用*/"ap2016012101112529","dataString"=>$dataString)));
    echo $iOSLink;
    ...
    // STEP 0. 账号帐户资料
    //更改商户把相关参数后可测试
    $APP_ID="wxb56a26391f370b0b";            //输入自己的 APPID
    $APP_SECRET="d99007bf2dd27e6fffeba604b004dea1";//输入自己的 appsecret
    //商户号，填写商户对应参数
    $MCH_ID="1277557401"; //输入自己的 MCH_ID
    //商户API密钥，填写相应参数
    $PARTNER_ID="189995"; //输入自己的 PARTNER_ID
    //支付结果回调页面
