...
@interface LYNetWorkSpeedMonitor ()

@property (assign,nonatomic) long long int bytesPerSecond;
@property (assign,nonatomic) LYNetWorkBytes networkBytesPerSecond;
...
@implementation LYNetWorkSpeedMonitor

+(LYNetWorkSpeedMonitor *)sharedMonitor {
    static LYNetWorkSpeedMonitor *sharedMonitor = nil;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        sharedMonitor = [[LYNetWorkSpeedMonitor alloc] init];
    });
...
- (void)monitorNetWorkSpeed {
    LYNetWorkBytes bytes = [self getBytes];
...
//获取每秒字节数
-(LYNetWorkBytes)getBytes
{
    static NSTimeInterval lastTime = 0;
    static LYNetWorkBytes lastNetworkBytes = {0,0};
    static NetworkStatus lastStatus = NotReachable;
    
    LYNetWorkBytes currentNetworkBytes = {0,0};
